...
**开源项目架构与特性设计分析 Skill：读懂业务逻辑，判断哪些值得借鉴。**

## 整体介绍

`opensource-aiproject-explorer` 是一份供 Agent 调用的分析 Skill。

当你面对一个开源项目——尤其是 AI、Agent、LLM 应用或相关项目——你可以用它来研究该开源项目，帮你了解以下两类问题：

1. **整体架构**：这个项目解决什么问题？一次请求怎样穿过模型、Agent、工具和状态层？模块之间如何协作？
2. **特性详细设计**：Memory、Planning、RAG、Tool Use、Sandbox、Skills 等关键能力为何存在、怎样运行、输入输出是什么、失败如何处理？
...
将 `opensource-aiproject-explorer/` 放到 Agent 可加载的 Skill 目录，安装和调用该skill即可。
...
```text
请使用 @opensource-aiproject-explorer 分析这个项目：
...
```text
分析 D:\code\<repo> 的整体架构，输出到 projects/<name>/
```
...
```text
详细分析一下它的 Memory 设计
它的Multi Agent 的架构是什么样的？各个Agent 之间是如何分工的，各个agent之间如何通信的？什么情况下会创建 Sub Agent？以及什么情况下会交给 Sub Agent 去做？
```
